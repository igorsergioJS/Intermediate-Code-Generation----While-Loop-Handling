...
void While::Gen() {
    unsigned begin = NewLabel();    // Cria um rótulo para o início do loop
    unsigned after = NewLabel();    // Cria um rótulo para o ponto de saída do loop

    cout << 'L' << begin << ':' << endl;  // Imprime o rótulo de início para a condição do loop
    Expression *condition = Rvalue(expr); // Avalia a condição do loop

    // Desvio condicional baseado na condição do loop
    cout << "\tifFalse " << condition->ToString() << " goto L" << after << endl;

    // Gera o código para o corpo do loop
    stmt->Gen();

    // Desvio de volta para o início para verificar a condição novamente
    cout << "\tgoto L" << begin << endl;

    // Imprime o rótulo que marca a saída do loop
    cout << 'L' << after << ':' << endl;
}
